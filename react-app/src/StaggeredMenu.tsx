...
export const StaggeredMenu = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (contentEls.length) {
      const contentStart = panelInsertTime + panelDuration * 0.2;
      tl.to(
        contentEls,
        {
          y: 0,
  ...
  const playClose = useCallback(() => {
    ...
    closeTweenRef.current = gsap.to(all, {
      ...
      onComplete: () => {
        ...
        const contentEls = Array.from(panel.querySelectorAll('.sm-panel-content > *'));
        if (contentEls.length) gsap.set(contentEls, { y: 24, opacity: 0 });
        busyRef.current = false;
      },
    });
  }, [position, offscreenX]);
  ...
  const handleItemActivate = (item: StaggeredMenuItem, event: React.MouseEvent<HTMLAnchorElement>) => {
    if (item.onClick) {
  ...
  const wrapperClass =
    ...
    (hideLogo ? ' staggered-menu-wrapper--hide-logo' : '');
  return (
    <div
    ...
    >
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        {!hideLogo && (
          <div className="sm-logo" aria-label="Logo">
            <img
              src={logoUrl || '/CirculationsLogoNoBg.png'}
              alt="Logo"
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
        <div className="sm-panel-inner">
          {children ? (
            <div className="sm-panel-content">{children}</div>
          ) : (
            <>
                  ...
                  items.map((it, idx) => (
                    <li className="sm-panel-itemWrap" key={it.label + idx}>
                      <a
                        ...
                        data-index={idx + 1}
                        onClick={e => handleItemActivate(it, e)}
                      >
